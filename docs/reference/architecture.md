...
The overview and node detail share one request owner. Hidden pages stop polling; expired sessions abort outstanding requests, and late responses cannot replace the current node. Vue-ECharts loads with the detail view, updates the same chart instance as data changes, and releases its observers and canvas when unmounted. Failure markers remain visible without latency data; ICMP loss and TCP connection failures retain distinct tooltip labels. Event lists display the latest five entries in the selected range without deleting stored history. The production build precompiles templates and retains the existing Content Security Policy.
