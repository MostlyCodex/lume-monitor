...
- `/status`：查看紧凑的最新状态；
- `/panel`：生成 5 分钟内有效、仅可使用一次的面板登录链接；
- `/help`：查看命令说明。
...
页面不会把所有时间范围都强行绘制成相同密度。默认显示分辨率为：
...
因此“每 60 秒上报”与“首页每 5 分钟一个格”并不冲突：前者是采集频率，后者只是全节点视图的传输和绘制聚合。进入单节点详情后，短周期历史按一分钟点位返回；两者都读取持续采集的数据，`/status` 和 `/panel` 不会临时发起探测。
