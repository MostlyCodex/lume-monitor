...
旧版本 Worker 缺少接管接口时，向导会询问是否更新现有 Worker 代码。这个步骤保留现有 D1 和 Secrets。已具备接口时，接管只读取线上数据。

清单包含已经登记但尚未上报的密钥；必须全部核对完成才能接管。密钥不一致或接管期间线上发生变化时会停止，不会覆盖其他节点。重新接管已有本地状态时，先自动保存备份。

## 新增 VPS

选择“新增 VPS”，填写节点 ID、名称、地区、可选观测和 SSH 别名：
...
选择节点后，可以调整 systemd 服务，保留、追加、删除或重新设置 ICMP/TCP 探针和 nftables 计数器，并立即部署。改动前会备份配置。三个可选列表为空时，仅采集主机资源。

| 观测 | 向导中填写 | 含义 |
| --- | --- | --- |
| systemd | 服务名，如 `nginx.service` | 只读服务运行状态 |
| ICMP | 目标主机名或 IP | 网络层延迟与 Echo 丢包 |
| TCP | 主机名或 IP、端口 | 建连延迟与建连失败率 |
| nftables | family、table、chain、协议、端口及可选 comment | 已有规则的命中增量；不创建或修改规则 |

节点间探针写在**发起探测的源节点**，目标节点 ID 填接收方的 ID。外部参考目标不填节点 ID。需要双向观测时，在两端分别配置。
