...
选择“新增 VPS”，填写节点信息，再配置 systemd 服务、网络质量监测和 SSH 别名：
...
网络质量有四个明确选项：**三网 ICMP 向导、复用已有节点的外部探针、自定义 ICMP/TCP、本次不添加**。已有节点配置过三网时，选“复用”，选择来源节点和探针即可。首台节点可在三网向导填写电信、联通、移动各一个参考目标；工具不内置未经确认的运营商 IP。向导会展示完整监测摘要，确认后直接部署；需要 sudo 密码时，在同一终端的 SSH 提示中输入。
...
批量增加：`npm run node:add -- --from-file ../nodes.json`。清单为 JSON 数组，每项包含 `id`，以及可选的 `name`、`role`、`region`、`ssh`、`services` 和 `probes`。
...
选择节点后，systemd 服务和网络探针分别配置。网络探针选择“追加”或“重新配置”后，进入同样的三网、复用和自定义向导。复用会展示实际目标供选择，不复制节点密钥或节点间探针。

确认摘要后，向导自动备份、保存并询问是否立即部署。保存只更新本地配置，部署到 VPS 后才生效；选择稍后部署或部署中断时，会标记为“配置待部署”。再次进入向导可直接保留现有配置并继续下发。两个可选列表为空时，仅采集主机资源。

每项输入都会显示可选值、格式或数量上限，并以分隔线区分。`/cancel` 或 Ctrl+C 取消当前操作；网络探针表单中 `/back` 返回上一项，回车保留已填内容。误选继续添加时，取消只丢弃未完成的一条，保留前面已填好的探针。三网和复用完成后直接进入配置摘要；内部探针名称自动生成。选择“重新配置”后再取消或选“本次不添加”，会保留原探针；清空请用“删除指定项”。

填错、重名、越界时，只提示重填当前项，保留此前输入；“是／否”只接受提示中列出的答案。显示名、用途和地区可自定义，仍须符合标明的长度限制。
...
节点间探针写在**发起探测的源节点**，目标节点 ID 只能从列出的其他活动节点中选择。外部参考目标选择 `0`。需要双向观测时，在两端分别配置。

ICMP 丢包和 TCP 建连失败含义不同，目标过滤 ICMP 也可能导致探测失败。[测量口径](reference/monitoring-methodology.md)说明指标定义和限制。

三网图表反映 VPS 到所选三个参考目标的延迟和丢包，历史从启用后积累。

## 部署配置与更新 Agent

选择“部署配置 / 更新 Agent”，或运行 `npm run node:apply`。工具上传当前本地配置和项目版本的 Agent，检查配置、备份现有文件，再更新。失败时由升级脚本回滚监控自身；运行中的业务服务不参与更新。

更新仓库或 Worker 后，已有 VPS 的 Agent 和监测配置仍需通过此操作下发。若三网已配置但面板没有显示，先运行 `npm run monitor:status`；显示“配置待部署”时，在菜单选择 **6「部署配置 / 更新 Agent」**，再选目标节点，也可直接运行（将 `hk-01` 替换为自己的节点 ID）：

```bash
npm run node:apply -- hk-01
```

需要 sudo 时，在 SSH 提示中输入 VPS 登录用户的密码。等待工具确认“Worker 已接受部署后的新上报”后刷新面板；三网历史从启用后开始积累。
...
点击面板右上角齿轮，可以修改面板名称、节点显示信息、卡片顺序和背景图片。

在“背景图片”中选择 JPG、PNG 或 WebP 图片（最大 10 MB、2400 万像素），查看横屏、竖屏预览，再点击“保存并应用”。图片会自动压缩、居中裁切，并适配深浅主题。点击“恢复默认背景”后保存可使用项目自带背景；取消或关闭设置会放弃本次修改。

显示设置仅保存在当前浏览器，图片不会上传，无需重新部署。更换浏览器或清除网站数据后需重新设置。
