...
test("cycle totals reuse the traffic row and explain the selected interfaces and observed period",async({page},testInfo)=>{
 ...
 await expect(card).toContainText("本周期流量");
 await card.click();
 const facts=page.locator("#detail-facts");await expect(facts).toContainText("eth0、ens192");await expect(facts).toContainText("已观测部分");
...
test("enabled accounting without a valid period never falls back to system totals",async({page})=>{
 ...
 await page.goto(`${origin}/dashboard/`,{waitUntil:"networkidle"});const card=page.locator(".node-card").first();
 await expect(card).toContainText("本周期流量");await expect(card.locator(".node-network-row").nth(1)).toContainText("↑ —");await card.click();await expect(page.locator("#detail-facts")).toContainText("统计暂不可用");
});
