  ...
  const nodes = catalog.nodes.map((meta) => {
    ...
    return {
      ...
      metrics: {
        ...
        traffic_cycle: report.system.traffic_cycle ?? null,
        traffic_cycle_enabled: report.system.traffic_cycle_enabled ?? Boolean(report.system.traffic_cycle),
        network_rx_bytes: report.system.network_valid === false ? null : Math.max(0, report.system.network_rx_bytes),
        network_tx_bytes: report.system.network_valid === false ? null : Math.max(0, report.system.network_tx_bytes),
      ...
      probes: report.probes
        .map((probe) => {
          ...
          return {
            ...
            packet_loss_percent: probe.kind === "icmp"
              ? safePercent(probe.packet_loss_percent ?? probe.sample_failure_percent)
              : null,
  ...
  if (selectedNode) {
    ...
    probeSummaries = [...grouped.entries()].map(([name, rows]) => {
      ...
      return {
        ...
        packet_loss_percent: meta?.kind === "tcp" ? null : failurePercent,
  ...
  return {
    ...
    probes: probeRows.map((row) => {
      ...
      return {
        ...
        category: meta?.category ?? "other",
        kind: meta?.kind ?? "icmp",
        packet_loss_percent: meta?.kind === "tcp" ? null : row.sample_failure_percent,
