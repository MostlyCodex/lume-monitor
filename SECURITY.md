...
The optional `yuanshanctl` manager retains the complete `NODE_KEYS` mapping,
generated node configurations, `ADMIN_TOKEN` and Webhook setup material under
the Git-ignored `.yuanshan/` directory so that later node additions cannot
...
`yuanshanctl node install` accepts a validated SSH alias, downloads only an
